...
export default function SolarFlareVisualizer(props: RendererProps) {
	...
	const onRender = useCallback((ctx: CanvasRenderingContext2D | null, data: CanvasData, state: RendererState) => {
		...
		try {
			...
			// Dessin de l'Éclipse Stellaire & Couronne
			drawSolarCoronaEclipse(ctx, cx, cy, radius, energyTime, bassEnergy, midEnergy, palette);
		} catch (err) {
			console.error("[Visualizer] SolarFlare render error:", err);
		}
	}, []);
