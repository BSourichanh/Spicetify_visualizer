...
export function getThemePalette(color: { r: number; g: number; b: number }, bassEnergy = 0, punch = 0): ThemePalette {
	const r = color?.r ?? 180;
	const g = color?.g ?? 180;
	const b = color?.b ?? 180;
	...
	const flash = Math.max(0, Math.min(1, effectivePunch * 0.8 + bassEnergy * 0.45));
	...
	return {
		...
		solid: `rgb(${r}, ${g}, ${b})`,
		glow: `rgba(${r}, ${g}, ${b}, ${Math.min(1, 0.4 + bassEnergy * 0.45 + effectivePunch * 0.35)})`,
		highlight: `rgb(${hr}, ${hg}, ${hb})`,
		veil: (alpha: number) => `rgba(${r}, ${g}, ${b}, ${Math.max(0, Math.min(1, alpha))})`,
		radialGrad: (
			ctx: CanvasRenderingContext2D,
			cx: number,
			cy: number,
...
export function getHarmonicPalette(colorInfo: { r: number; g: number; b: number }, bassEnergy: number, punch = 0) {
	const p = getThemePalette(colorInfo, bassEnergy, punch);
	return {
