...
export default function NeonTunnelVisualizer(props: RendererProps) {
	...
	const onRender = useCallback((ctx: CanvasRenderingContext2D | null, data: CanvasData, state: RendererState) => {
		...
		try {
			...
			// Dessin du Trou Noir & Disque d'Accrétion
			drawBlackHoleAccretion(ctx, cx, cy, radius, energyTime, bassEnergy, midEnergy, palette);
		} catch (err) {
			console.error("[Visualizer] NeonTunnel render error:", err);
		}
	}, []);
