...
export default function NeonWavesVisualizer(props: RendererProps) {
	const onError = useContext(ErrorHandlerContext);

	const audioAnalysis = useMemo(() => {
		const result = props.trackData.audioAnalysis;
		if (result?.error) console.warn("[Visualizer] NeonWaves:", result.error);
		return result?.value;
	...
	const onRender = useCallback((ctx: CanvasRenderingContext2D | null, data: CanvasData, state: RendererState) => {
		...
		try {
			...
			const { bassEnergy, midEnergy, energyTime, punch } = features;
			...
			// Dessin de la Nébuleuse Cosmique
			drawCosmicNebula(ctx, width, height, energyTime, bassEnergy, midEnergy, palette);
		} catch (err) {
			console.error("[Visualizer] NeonWaves render error:", err);
		}
	}, []);
