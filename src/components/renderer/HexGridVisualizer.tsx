...
export default function HexGridVisualizer(props: RendererProps) {
	...
	const onRender = useCallback((ctx: CanvasRenderingContext2D | null, data: CanvasData, state: RendererState) => {
		...
		try {
			...
			// Dessin des Biocellules Luminescentes
			drawBioluminescentBiocells(ctx, width, height, energyTime, bassEnergy, palette);
		} catch (err) {
			console.error("[Visualizer] HexGrid render error:", err);
		}
	}, []);
