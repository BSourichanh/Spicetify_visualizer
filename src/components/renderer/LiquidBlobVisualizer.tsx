...
export default function LiquidBlobVisualizer(props: RendererProps) {
	...
	const onRender = useCallback((ctx: CanvasRenderingContext2D | null, data: CanvasData, state: RendererState) => {
		...
		try {
			...
			// Dessin de la Goutte d'Aura Abyssale
			drawBioluminescentWaterdrop(ctx, cx, cy, radius, energyTime, bassEnergy, trebleEnergy, palette);
		} catch (err) {
			console.error("[Visualizer] LiquidBlob render error:", err);
		}
	}, []);
