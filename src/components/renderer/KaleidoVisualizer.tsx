...
export default function KaleidoVisualizer(props: RendererProps) {
	...
	const onRender = useCallback((ctx: CanvasRenderingContext2D | null, data: CanvasData, state: RendererState) => {
		...
		try {
			...
			// Dessin du Lotus Astral
			drawAstralLotus(ctx, cx, cy, radius, energyTime, 0, bassEnergy, trebleEnergy, palette);
		} catch (err) {
			console.error("[Visualizer] Kaleido render error:", err);
		}
	}, []);
