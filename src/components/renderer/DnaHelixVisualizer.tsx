...
export default function DnaHelixVisualizer(props: RendererProps) {
	...
	const onRender = useCallback((ctx: CanvasRenderingContext2D | null, data: CanvasData, state: RendererState) => {
		...
		try {
			...
			// Dessin de la Double Hélice de Soie
			drawFluidSilkHelix(ctx, width, height, energyTime, bassEnergy, midEnergy, palette);
		} catch (err) {
			console.error("[Visualizer] DnaHelix render error:", err);
		}
	}, []);
