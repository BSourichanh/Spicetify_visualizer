...
export default function LissajousVisualizer(props: RendererProps) {
	...
	const onRender = useCallback((ctx: CanvasRenderingContext2D | null, data: CanvasData, state: RendererState) => {
		...
		try {
			...
			// Dessin des Anneaux Célestes de Saturne
			drawSaturnCelestialRings(ctx, cx, cy, radius, energyTime, bassEnergy, midEnergy, palette);
		} catch (err) {
			console.error("[Visualizer] Lissajous render error:", err);
		}
	}, []);
