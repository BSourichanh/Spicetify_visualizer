...
export default function StarfieldVisualizer(props: RendererProps) {
	...
	const onRender = useCallback((ctx: CanvasRenderingContext2D | null, data: CanvasData, state: RendererState) => {
		...
		try {
			...
			// Dessin de la Galaxie Spirale
			drawSpiralGalaxy(ctx, cx, cy, radius, energyTime, bassEnergy, midEnergy, palette);
		} catch (err) {
			console.error("[Visualizer] Starfield render error:", err);
		}
	}, []);
