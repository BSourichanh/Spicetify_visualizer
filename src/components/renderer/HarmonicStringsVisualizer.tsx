...
export default function HarmonicStringsVisualizer(props: RendererProps) {
	...
	const onRender = useCallback((ctx: CanvasRenderingContext2D | null, data: CanvasData, state: RendererState) => {
		...
		try {
			...
			// Dessin des Ondes de Soie Harmoniques
			drawHarmonicSilkWaves(ctx, width, height, energyTime, bassEnergy, midEnergy, palette);
		} catch (err) {
			console.error("[Visualizer] HarmonicStrings render error:", err);
		}
	}, []);
